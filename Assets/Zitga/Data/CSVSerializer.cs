...
namespace Loxodon.Framework.Data
{
    public static class CSVSerializer
    {
        ...
        {
            return (T) CreateIdValue(typeof(T), ParseCSV(text), id_col,
                value_col);
        }

        static public T DeserializeIdValue<T>(List<string[]> rows, int id_col = 0, int value_col = 1)
        {
            return (T) CreateIdValue(typeof(T), rows, id_col,
                value_col);
        }
        ...
        static object CreateIdValue(Type type, List<string[]> rows, int id_col = 0,
            int val_col = 1)
        {
            object v = Activator.CreateInstance(type);
